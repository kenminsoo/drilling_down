...
for dataset in phenotypic_data:
    ...
    for field in phenotypic_fields[dataset]:
        phenotypic_codes[dataset][field] = phenotypic_codes_raw[dataset][ph_n]['code'].split('\t')
        ph_n += 1

#store samples for data, implement a way to add samples from multiple datasets
genomic_samples = {}
phenotypic_samples = {}
all_samples = []
...
def add_phenotypes(dictionary_to_add_phenotypes):
    

#testing fx
house_keepers = extract_xena_data_g(genomic_samples, house_keepers)
goi = extract_xena_data_g(genomic_samples, genes_of_interest)
prev_est = extract_xena_data_g(genomic_samples, prev_est_GOI)
phenotype = extract_xena_data_p(phenotypic_samples, phenotypic_fields)


exp_gene_data = [goi, house_keepers,prev_est]
all_3 = combine_dicts(exp_gene_data)

print(all_3['sample'])
print(all_3['GTEX-S4Q7-0003-SM-3NM8M'])
print(all_3['TCGA-EK-A2RE-01'])
print(all_3['GTEX-T5JC-0011-R11A-SM-32PMB'])