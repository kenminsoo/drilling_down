...
import csv
#version v1_2 notes:
#this version has only been tested to work with one dataset at a time with one API
#universal gene code mapping has not been implemented
#future features:
### Combine multiple datasets, better query support, gene decoding with universal code, GUI
...
with open('house_keepers.csv', 'w') as f:
    for key in house_keepers.keys():
        f.write("%s, %s\n"%(key, house_keepers[key]))

with open('goi.csv', 'w') as f:
    for key in goi.keys():
        f.write("%s, %s\n"%(key, goi[key]))

with open('prev_est.csv', 'w') as f:
    for key in prev_est.keys():
        f.write("%s, %s\n"%(key, prev_est[key]))

with open('goi_hk_prev_genomic_data.csv', 'w') as f:
    for key in goi_hk_prev_genomic_data.keys():
        f.write("%s, %s\n"%(key, goi_hk_prev_genomic_data[key]))

with open('decoded_phenotype.csv', 'w') as f:
    for key in decoded_phenotype.keys():
        f.write("%s, %s\n"%(key, decoded_phenotype[key]))
